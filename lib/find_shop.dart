...
class _FindShopState extends State<FindShop> {
  ...
  LatLng _currentLocation = const LatLng(13.7563, 100.5018); // Victory Monument, Bangkok, Thailand
  ...
  void _markNearestShop() async {
    try {
      ...
      final response = await dio.get(dotenv.env['GOOGLE_PLACE_API_ENDPOINT']!, queryParameters: params);
        ...
        setState(() {
          _markers.add(
            Marker(
              ...
              position: LatLng(results[i]['geometry']['location']['lat'], results[i]['geometry']['location']['lng']),
